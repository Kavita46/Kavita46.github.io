...
1. 需求来源
  ...
  使用github pages 和 jkelly 是因为简单易上手，就算不懂编程也能玩明白，也不像wordpress 这些需要自建服务器。所以暂时先用来实现一些想法。将博文以 markdown 格式上传至博客的时候，需要添加文件头，文件太多的时候手动操作显得很呆，所以利用学过的JAVA IO相关知识，通过每次改变一些参数，实现文件自动更名和添加文件头。
